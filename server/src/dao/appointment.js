...
class AppointmentDAO {
  ...
  async getDoctorAppointmentsByDate(id, date) {
    const response = await db("appointments").where({
      doctor_id: id,
      date: date,
    });
    return response;
  ...
  async fixAppointment(doctor_id, patient_id, date, time) {
    const [response] = await db("appointments")
      .insert({
        doctor_id: doctor_id,
  ...
  async getAvailableTimeslots(doctor_id, patient_id, date) {
    const response = await db("appointments")
      .where({ patient_id: patient_id, date: date })
      .orWhere({ doctor_id: doctor_id, date: date })
      .returning("time");
